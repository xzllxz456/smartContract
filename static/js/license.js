...
//     const admin1Exists = await wallet.exists('admin2');
//     if (!admin1Exists) {
...
router.get('/wallet', async(req, res, next) => {
    try{
        ...
        if (!userExists) {
            ...
            const secret = await ca.register({ affiliation: 'org1.department1', enrollmentID: 'user2', role: 'client' }, admin1Identity);
            ...
            console.log('Successfully registered and enrolled admin2 user "user2" and imported it into the wallet');
        }
...
router.post('/createLicense', async (req, res, next) => {
    try{
    ...
    const userExists = await wallet.exists('user2');
        ...
        await contract.submitTransaction('CreateLicense',`${req.body.key}`,`${req.body.cnumber}`,`${req.body.userid}`,`${req.body.policy1}`,`${req.body.policy2}`,`${req.body.policy3}`,`${req.body.userprofil}`);
...
router.post('/searchquery', async (req, res, next) => {
    try{
        ...
        const network = await gateway.getNetwork('testchannel1');
        const contract = network.getContract('license');
        const result = await contract.evaluateTransaction('ReadLicense', `${req.body.key}`);
...
//queryAll
router.get('/queryAlllicense', async (req, res, next) => {
    try{
